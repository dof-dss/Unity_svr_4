...
configset per site. Unity 4 currently declares 10 Solr-hosted sites in
`project/project.yml`; 7 have a committed Drupal `solr_default` server.

Three hosting declarations (`anotherway`, `oice`, and `cusubt`) do not currently have a committed Drupal `solr_default` server. Their existing configsets are preserved, but the generator intentionally skips them until that application configuration exists.
